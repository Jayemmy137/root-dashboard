...
import { findPlant, listPlantNames, detectIntent, NARROW_INTENTS } from "@/lib/plantData";
...
// Core message handling, shared by the input form and the quick chips.
// Narrow, single-field intents (moisture / frequency / tips / issues) and
// meta intents (list / greeting) get a plain text reply. Anything else
// that matches a plant falls through to the general/full-info reply,
// which carries the `plant` object — that's what renders the card and
...
{
role: "bot",
text: "Hello! I can tell you the watering needs, ideal soil moisture, and common issues for vegetables I know about. Try: \"how often should I water okra?\"",
},
]);
return;
}
...
setMessages((prev) => [
...prev,
{ role: "bot", text: "I currently have data on: " + listPlantNames().join(", ") + "." },
]);
return;
}

const match = findPlant(lower);
...
if (NARROW_INTENTS.includes(intent)) {
let narrowText = "";
switch (intent) {
case "moisture":
narrowText = `${match.name} does best with soil moisture between ${match.minMoisture}% and ${match.maxMoisture}%. Below that range, consider triggering irrigation.`;
break;
case "frequency":
narrowText = `${match.name}: ${match.frequency}`;
break;
case "tips":
narrowText = `Tip for ${match.name}: ${match.tips}`;
break;
case "issues":
narrowText = `Common issue with ${match.name}: ${match.issues}`;
break;
}
// No `plant` field here on purpose — narrow replies don't get the
// full card or the Apply button.
setMessages((prev) => [...prev, { role: "bot", text: narrowText }]);
return;
}

// General / full-info reply — this is the only branch that attaches
// `plant`, which is what triggers the card + Apply button in the UI.
setMessages((prev) => [
...prev,
{
role: "bot",
...
className="text-xs px-3 py-1.5 rounded-full border border-[var(--border-color)] text-[var(--text-muted)] bg-[var(--bg-card)] hover:border-[var(--accent)] hover:text-[var(--accent)] transition-colors"
>
{chip}
</button>
))}
